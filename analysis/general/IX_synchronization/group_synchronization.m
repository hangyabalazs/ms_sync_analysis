...
%   GROUP_SYNCHRONIZATION(ROWIDS,GRPID,FRBANDS) calculates 7 measures for
...
%   - less theta skipping
%   - frequency sycnhronization, frequency deviations.
%   Parameters:
...
save(fullfile(RESULTDIR,grpId,'frequency_synchronization'),...
    'thPoints','dePoints','IDs','nPoints');
%% Rhythmicity frequency deviations between corecorded pairs (6. measure):
[thPoints,dePoints,nPoints,IDs] = rh_frequency_deviations(rowIds,frBands);
thPoints = abs(thPoints);
dePoints = abs(dePoints);
save(fullfile(RESULTDIR,grpId,'rh_frequency_deviations_abs'),...
    'thPoints','dePoints','IDs','nPoints');
end
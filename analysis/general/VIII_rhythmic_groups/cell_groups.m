...
%   EXAMPLE_CELLS_PLOT, RHYTHMICITY_BAR.
...
if exist('trgtFolder','var')
    ...
    savefig(H4,fullfile(RESULTDIR,trgtFolder,'phasePrefTheta.fig'));
    savefig(H5,fullfile(RESULTDIR,trgtFolder,'PhasePrefDelta.fig'));
    % Collect all plots:
%     F = figure('Position',get(0,'Screensize')); 
    F = figure('Position',[1,41,1097,505]); 
    s = subplot(2,7,1:3); copy_figure_to_subplot(H1,F,s), xlim([(CGWINDOW-2)*NSR,(CGWINDOW+2)*NSR])
    s = subplot(2,7,8:10); copy_figure_to_subplot(H2,F,s), xlim([(CGWINDOW-2)*NSR,(CGWINDOW+2)*NSR])
    s = subplot(2,7,4:5); copy_figure_to_subplot(H3,F,s), xlim([(CGWINDOW-2)*NSR,(CGWINDOW+2)*NSR])
    s = subplot(2,7,11:12); copy_figure_to_subplot(H4,F,s)
    s = subplot(2,7,6:7); copy_figure_to_subplot(H5,F,s), axis equal
    s = subplot(2,7,13:14); copy_figure_to_subplot(H6,F,s), axis equal
    grpName = trgtFolder(find(trgtFolder=='\')+1:end);
    savefig(fullfile(RESULTDIR,trgtFolder,[grpName,'.fig']));
    close([H1,H2,H3,H4,H5,H6,F])
end
end
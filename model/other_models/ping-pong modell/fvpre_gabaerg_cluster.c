...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 void _fvpre_gabaerg_cluster_reg() {
	int _vectorized = 0;
  _initlists();
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 6, 4);
 	...
 	ivoc_help("help ?1 fvpre C:/Users/kocsis.barnabas/Desktop/modelling/Neuron_software/ping-pong modell/fvpre_gabaerg_cluster.mod\n");
...
double F (  double _lv1 ) {
   double _lF;
 _lF = 1.0 / ( 1.0 + exp ( - ( _lv1 - thetasyn ) / 2.0 ) ) ;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  state();
 if(error){fprintf(stderr,"at line 39 in file fvpre_gabaerg_cluster.mod:\n  SOLVE state METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
